...
contract NFTMarket is ReentrancyGuard {
    ...
    ICollectionDeployer _collectiondeployer;

    constructor(address adr) {
        owner = payable(msg.sender);
        _collectiondeployer = ICollectionDeployer(adr);
    }
    ...
    function createCollection(
        ...
        string memory symbol,
        address colOwner
    ) public returns (address adr) {
        address collectionaddress;
        collectionaddress = _collectiondeployer.createCollection(
            name,
            symbol,
            colOwner
        );

        collections.push(collection(name, symbol, collectionaddress));
